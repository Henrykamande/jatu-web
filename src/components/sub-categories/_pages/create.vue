  ...
  <v-container fluid>
    <v-card>
      <v-toolbar color="secondary" flat dense>
        <v-toolbar-title><span style="color: red">{{selectedCategory.name}}</span> -> New Sub Category</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-btn color="primary" small text :to="`/category/sub-categories/${selectedCategory.url}`">
  ...
  methods: {
    ...
    save(data) {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.state) {
            self.$router.push(`/category/sub-categories/${this.selectedCategory.url}`);
          } else {
