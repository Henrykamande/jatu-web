  ...
  <v-container fluid>
    <v-card>
      <v-toolbar color="secondary" flat dense>
        <v-toolbar-title><span style="color: red">{{selectedCategory.name}}</span> -> Sub Categories</v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-row>
          <v-col cols="12" xs="12" sm="12" md="6">
            <v-btn color="accent" small :to="`/new-sub-category/${selectedCategory.url}`">
              <v-icon left dark>mdi-plus</v-icon>New Sub Category
            </v-btn>
          </v-col>
        </v-row>
  ...
  methods: {
    ...
    save(data) {
     ...
     getData(categoryUrl) {
      const self = this;
      this.$store
        .dispatch("get", `/subcategories/select/${categoryUrl}`)
        .then((res) => {
