...
export default {
  ...
  methods: {
    ...
    addPricing() {
      const priceRecord = this.sizes.find((record) => {
        return record.url === this.volume;
      });

      const pos = this.sizes.indexOf(priceRecord);
    ...
    changeStatus(val) {
      if (this.action == "view") {
        let data = { outOfStock: val };
        const url = `/products/update/stock/status/${this.record._id}`;
        const self = this;
        this.$store
          .dispatch("expressPut", { url, data })
          .then((res) => {
    ...
    sendData() {
      ...
      formData.append("giftpack", false);
