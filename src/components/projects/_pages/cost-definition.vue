  ...
  <v-container fluid>
    <v-card>
      <v-toolbar color="secondary" flat dense>
        <v-toolbar-title>
          <span style="color: red">{{dataItem.title}}</span> - Costs Definition
        </v-toolbar-title>
        <v-spacer></v-spacer>
          ...
          <v-col cols="12" md="8">
            ...
            >
              <template v-slot:item.action="{ item }">
                <v-btn small color="blue" text :to="`/project/costs/${dataItem.url}/${item.url}`">
                  <v-icon left>mdi-cog-outline</v-icon>Manage Cost Items
                </v-btn>
              </template>
            </v-data-table>
          </v-col>
