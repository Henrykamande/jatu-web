      ...
      <v-card-text>
        <v-row>
          <v-col cols="12" xs="12" sm="12" md="6">
            <v-btn color="accent" small to="/new-project-category">
              <v-icon left dark>mdi-plus</v-icon>New Project Category
            </v-btn>
          </v-col>
        </v-row>
          ...
          <v-col cols="12" sm="12" md="12">
            ...
            >
              ...
              <template v-slot:item.action="{ item }">
                <v-btn color="editIcon" text :to="`/project-category/${item.url}`" icon>
    ...
    <v-dialog v-model="deleteDialog" max-width="650px">
      <v-card>
        <v-toolbar dense color="accent" dark>
          <v-toolbar-title>Delete Project Product</v-toolbar-title>
          <v-spacer></v-spacer>
        ...
        <v-card-text>
          <v-container fluid>
            <v-row dense class="subtitle-2 text--primary">
              <v-col cols="12" sm="12" md="12">
                <span>
                  Are you sure you want to delete the following Project category ?</span
                >
              </v-col>
...
export default {
  ...
  methods: {
    getCategories() {
      const self = this;
      this.$store
        .dispatch("get", `/project-categories`)
        .then((res) => {
          if (res.state) {
            console.log ("res==============",res)
    ...
    deleteCategory() {
      ...
      this.loader = true;
      this.$store
        .dispatch("remove", `/project-categories/delete/${this.deleteItem.url}`)
        .then((res) => {
          if (res.state) {
            self.loader = false;
            location.reload();
