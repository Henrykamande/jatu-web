<template>
  <div class="container mx-auto md:px-80 px-2 lg:min-h-screen">
    <div class="bg-white mt-6 p-3">
      <div>
        <div class="bg-orange-600 text-white font-bold px-6 py-2">Order farm</div>
        <div>
            ...
            <table class="w-full text-sm md:text-base text-left border-2 border-white">
              ...
              <tbody>
                <tr class="border-b dark:bg-gray-800 dark:border-gray-900">
                  <td class="border-2 border-white">Farm No</td>
                  <td class="border-2 border-white">{{ record.serialNo }}</td>
                </tr>
                <tr class="border-b dark:bg-gray-800 dark:border-gray-900">
                  <td class="border-2 border-white">Country</td>
                  <td class="border-2 border-white">{{ record.country && record.country.name }}</td>
                </tr>
                <tr class="border-b dark:bg-gray-800 dark:border-gray-900">
                  <td class="border-2 border-white">Zone</td>
                  <td class="border-2 border-white">{{ record.zone && record.zone.name }}</td>
                </tr>
                <tr class="border-b dark:bg-gray-800 dark:border-gray-900">
                  <td class="border-2 border-white">Acres Available</td>
                  <td class="border-2 border-white">{{ record.acres }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="mt-4">
            ...
            <div class="mt-4">
              <input
                v-model="eventRecord.acres"
                placeholder="Acres to order"
                type="number"
                class="w-full border-2 border-gray-400 py-1 px-3 rounded outline-none focus:border-purple-500"
              />
            </div>
  ...
  methods: {
    async joinEvent() {
      if (
        ...
        this.eventRecord.acres == undefined ||
